...
function initMap() {
    ...
    L.marker([lat, lng]).addTo(map).bindPopup(props.tenant.name).openPopup();
...
const approveForm = useForm({
    plan_slug: props.tenant.meta?.selected_plan ?? "starter",
});
function approve() {
    approveForm.patch(
        route("super-admin.verifications.approve", props.tenant.id),
    );
}
...
function reject() {
    rejectForm.patch(
        route("super-admin.verifications.reject", props.tenant.id),
        {
            onSuccess: () => {
    ...
    <SuperAdminLayout title="Verifications">
        <div class="p-6 space-y-6 max-w-4xl mx-auto">
            ...
            <div class="flex items-center gap-2 text-sm text-slate-500">
                <Link
                    :href="route('super-admin.verifications.index')"
            ...
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <span
                        class="text-xs rounded px-2 py-0.5 font-medium mt-1 inline-block"
                        :class="statusClass(tenant.verification_status)"
                    >
                        {{
                ...
                <div
                    v-if="tenant.verification_status === 'pending_verification'"
                    class="flex gap-3"
                >
                    <button
            ...
            >
                <div class="px-5 py-3 border-b border-slate-100">
                    <h2 class="font-semibold text-slate-700">
                        Business Location & Delivery Radius
                    </h2>
                </div>
                <div v-if="tenant.latitude">
                    ...
                    <div class="px-5 py-3 text-sm text-slate-500 flex gap-6">
                        <span
                            >Lat: <strong>{{ tenant.latitude }}</strong></span
                        >
                        <span
                            >Lng: <strong>{{ tenant.longitude }}</strong></span
                        >
                        <span
                            >Radius:
                            <strong
                                >{{ tenant.delivery_radius_km }} km</strong
                            ></span
                        >
                    </div>
                </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                >
                    ...
                    <div class="text-sm space-y-1.5">
                        <p>
                            <span class="text-slate-400 w-28 inline-block"
                                >Name</span
                        ...
                        <p>
                            ...
                            >
                            <span
                                class="bg-indigo-50 text-indigo-700 text-xs rounded px-2 py-0.5"
                                >{{
                ...
                >
                    ...
                    >
                        <label
                            ...
                            >Override Plan on Approval</label
                        >
                        <input
                        ...
                        />
                        <p
                            v-if="approveForm.errors.plan_slug"
                            class="text-red-500 text-xs mt-1"
                        >
                            {{ approveForm.errors.plan_slug }}
                        </p>
                    </div>
            ...
            >
                <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                    ...
                    <p class="text-sm text-slate-500 mb-4">
                        Please provide a reason. This will be sent to the
                        business owner.
                    </p>
                    <textarea
